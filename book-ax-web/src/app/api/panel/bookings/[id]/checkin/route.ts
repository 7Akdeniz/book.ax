...
export async function PATCH(
  ...
  try {
    ...
    // Check if booking can be checked in (must be confirmed)
    ...
    console.log(
      `[HOTELIER] Booking ${bookingId} checked in by hotelier ${payload.email}`
    );

    return NextResponse.json({ booking: updatedBooking });
  } catch (error) {
    console.error('Error in PATCH /api/panel/bookings/[id]/checkin:', error);
