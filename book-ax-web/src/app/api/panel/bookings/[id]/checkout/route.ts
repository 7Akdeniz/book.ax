...
export async function PATCH(
  ...
  try {
    ...
    // Check if booking can be checked out (must be checked_in)
    ...
    console.log(
      `[HOTELIER] Booking ${bookingId} checked out by hotelier ${payload.email}`
    );

    return NextResponse.json({ booking: updatedBooking });
  } catch (error) {
    console.error('Error in PATCH /api/panel/bookings/[id]/checkout:', error);
