-- ============================================
-- Book.ax Supabase Database Schema
-- ============================================
-- 
-- Kopiere dieses komplette SQL-Skript und füge es in deinen
-- Supabase SQL Editor ein (Dashboard → SQL Editor → New Query)
--
-- Dieses Skript erstellt:
-- 1. Users Table (erweitert die Auth-User)
-- 2. Hotels Table
-- 3. Bookings Table
-- 4. Reviews Table
...
-- 1. USERS TABLE
-- ============================================
-- Erweitert die Supabase Auth User mit zusätzlichen Profil-Infos
...
-- RLS Policies: Users können nur ihr eigenes Profil sehen/bearbeiten
...
-- RLS Policies: Hotels sind öffentlich lesbar
CREATE POLICY "Hotels are viewable by everyone"
  ON hotels FOR SELECT
  USING (true);

-- Nur Admins können Hotels erstellen/bearbeiten (später hinzufügen)
CREATE POLICY "Only admins can insert hotels"
  ON hotels FOR INSERT
  WITH CHECK (false); -- Später: auth.uid() IN (SELECT id FROM admin_users)
...
-- RLS Policies: User sieht nur eigene Buchungen
...
CREATE TABLE IF NOT EXISTS reviews (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE(hotel_id, user_id) -- User kann nur 1 Review pro Hotel schreiben
);

-- Enable RLS
ALTER TABLE reviews ENABLE ROW LEVEL SECURITY;

-- RLS Policies: Reviews sind öffentlich lesbar
...
-- ============================================
-- 5. AUTOMATISCHES RATING UPDATE
-- ============================================
-- Wenn eine neue Review erstellt wird, aktualisiere Hotel-Rating
...
-- ============================================
-- 6. GEO-SPATIAL QUERIES (Optional)
-- ============================================
-- Für "Hotels in meiner Nähe" Feature

-- PostGIS Extension aktivieren (wenn noch nicht)
CREATE EXTENSION IF NOT EXISTS postgis;

-- Function: Find hotels near location
...
-- ============================================
-- 7. SAMPLE DATA (Optional - zum Testen)
-- ============================================

-- Beispiel-Hotels einfügen (nur wenn Tabelle leer ist)
...
-- 3. Kopiere dieses komplette Skript
-- 4. Klicke "Run" (F5)
-- 5. Überprüfe die Tabellen in: Database → Tables
--
-- Dann in deiner App:
-- 1. Öffne .env
-- 2. Füge SUPABASE_URL und SUPABASE_ANON_KEY ein
